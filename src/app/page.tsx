      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-2 mb-4 justify-center items-center">
          ...
          {['user', 'blog', 'product'].map((type) => (
            <button
              ...
              className="text-[10px] px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:border-indigo-500 hover:text-indigo-600 dark:text-slate-300 transition font-bold uppercase shadow-sm"
            >
              {type}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className={`p-6 rounded-2xl shadow-sm border-2 transition-all bg-white dark:bg-slate-900 ${error ? 'border-red-300' : 'border-slate-200 dark:border-slate-800'}`}>
              <div className="flex justify-between items-center mb-4">
                <h2 className="font-bold text-slate-800 dark:text-slate-200 uppercase text-xs tracking-wider">Source JSON</h2>
                <div className="flex gap-2">
                  <button onClick={prettifyJson} disabled={!!error} className="text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-indigo-50 dark:bg-slate-800 text-indigo-600 rounded-md hover:bg-indigo-600 hover:text-white transition">Format</button>
                  <button onClick={handleReset} className="text-[10px] font-black uppercase tracking-widest px-3 py-1 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-md hover:bg-red-600 hover:text-white transition">Reset</button>
                </div>
              </div>
              <input
          ...
          <div className="rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col bg-white dark:bg-slate-900">
            <nav className="flex bg-slate-50 dark:bg-slate-800/50 border-b dark:border-slate-800">
              {['migration', 'model', 'factory', 'validation'].map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white dark:bg-slate-900 text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-400'}`}>
                  {tab}
                </button>
              ))}
            </nav>
        ...
        {shareUrl && (
          <div className="mt-8 p-6 bg-emerald-50 dark:bg-emerald-900/10 border-2 border-emerald-200 dark:border-emerald-800 rounded-2xl animate-in fade-in slide-in-from-top-4 duration-500">
            <p className="text-[11px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-widest mb-3">Public Link Generated:</p>
            <div className="flex gap-2">
              <input readOnly value={shareUrl} className="flex-1 bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-800 rounded-xl p-3 text-xs font-mono text-emerald-800 dark:text-emerald-300 outline-none" />
              ...
              <a href={shareUrl} target="_blank" className="p-3 bg-white dark:bg-slate-800 border border-emerald-200 dark:border-emerald-800 text-emerald-600 rounded-xl hover:bg-emerald-100 transition shadow-sm">Preview</a>
            </div>
          </div>
        )}
        ...
        <section className="mt-20 grid grid-cols-1 md:grid-cols-3 gap-10 border-t dark:border-slate-800 pt-10">
          <div className="text-center">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-2">⚡ Instant Conversion</h3>
            <p className="text-sm text-slate-500">Generate migrations and models in seconds. Optimized for Laravel 11.</p>
          </div>
          <div className="text-center">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-2">🛡️ PSR-12 Standard</h3>
            <p className="text-sm text-slate-500">Clean, production-ready code following PHP community standards.</p>
          </div>
          <div className="text-center">
            <h3 className="font-bold text-slate-800 dark:text-slate-200 mb-2">🔗 Shareable Schemas</h3>
        ...
        <section className="mt-20 max-w-4xl mx-auto prose dark:prose-invert prose-slate">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-6">How to use LaraQuick: The Ultimate JSON to Laravel Guide</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div>
              <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-3">Common JSON Formatting Errors</h3>
            ...
            <div>
              <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-3">Pro Tips for Developers</h3>
              <ul className="text-sm text-slate-600 dark:text-slate-400 space-y-2 list-disc pl-5">
          ...
          {/* --- NUEVA SECCIÓN: LISTA DE COMANDOS LARAVEL --- */}
          <div className="mt-16 border-t dark:border-slate-800 pt-10">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-6 tracking-tighter">Essential Laravel Commands</h2>
        ...
        <footer className="mt-20 text-center border-t border-slate-200 dark:border-slate-800 pt-10 pb-10">
          <div className="flex flex-wrap justify-center gap-x-10 gap-y-4 mb-8">
            ...
            <Link href="/terms" className="text-[11px] font-bold text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 uppercase tracking-widest transition">Terms</Link>
          </div>
